...
from yt_comments.analysis.channel_tfidf.paths import channel_tfidf_keywords_path
...
class ParquetChannelTfidfKeywordsRepository:
    """
    Gold TF-IDF keywords repository: 1 row per channel_id.

    Layout:
      <data_root>/gold/channel_tfidf/<channel_id>/keywords.parquet
    """
    def __init__(self, data_root: str | Path = "data") -> None:
        self._data_root = Path(data_root)
        
    def save(self, keywords: ChannelTfidfKeywords) -> None:
        path = channel_tfidf_keywords_path(self._data_root, keywords.channel_id)
        path.parent.mkdir(parents=True, exist_ok=True)
        table = self._to_table(keywords)
        pq.write_table(table, path)
        
    def load(self, channel_id: str) -> ChannelTfidfKeywords:
        path = channel_tfidf_keywords_path(self._data_root, channel_id)
        if not path.exists():
    ...
    @classmethod
    def _to_table(cls, item: ChannelTfidfKeywords) -> pa.Table:
    ...
    @classmethod
    def _from_table(cls, table: pa.Table) -> ChannelTfidfKeywords:
        if table.num_rows != 1:
        ...
        keywords: tuple[TfidfKeyword, ...] = tuple(keywords_list) 
