...
class ParquetTfidfKeywordsRepository:
    ...
    def _to_table(cls, item: TfidfKeywords) -> pa.Table:
        ...
        data = {
            ...
            "drop_numeric": [cfg.drop_numeric_tokens],
            "drop_stopwords": [cfg.drop_stopwords],
            "lang": [cfg.lang],
            "lowercase": [cfg.lowercase],
            "min_df_raw": [cfg.min_df_raw],
            "max_df_raw": [cfg.max_df_raw],
    ...
    def _from_table(cls, table: pa.Table) -> TfidfKeywords:
        ...
        cfg = TfidfConfig(
            ...
            drop_numeric_tokens=bool(row["drop_numeric"]),
            drop_stopwords=bool(row["drop_stopwords"]),
            lang=str(row["lang"]),
            lowercase=bool(row["lowercase"]),
            min_df_raw=int(row["min_df_raw"]),
            max_df_raw=int(row["max_df_raw"]),
    ...
    def _schema() -> pa.Schema:
        ...
        return pa.schema(
            [
                ...
                ("keep_numeric", pa.bool_()),
                ("keep_stopwords", pa.bool_()),
                ("lang", pa.string()),
                ("lowercase", pa.bool_()),
